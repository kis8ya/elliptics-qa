...
def test_pwc_psize_more_than_csize_negative(timestamp, user_flags):
    ...
    testhelper.write_commit_sync(key, data[2], len(data[0]) + len(data[1]), commit_size)
...
@pytest.mark.pwctest
def test_pwc_psize_less_than_plainsize_negative():
    """ Testing prepare-write-commit
...
def test_pwc_psize_less_than_csize_negative():
    ...
    assert_that(calling(testhelper.write_commit_sync).with_args(key, data[2], len(data[0]) + len(data[1]), data_len),
                raises(elliptics.Error, testhelper.error_info.WrongArguments))
...
def test_pwc_psize_less_than_csize():
    ...
    assert_that(calling(testhelper.write_commit_sync).with_args(key, data[2], len(data[0]) + len(data[1]), data_len + add_len),
